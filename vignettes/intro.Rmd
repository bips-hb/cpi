...
```
The result is a CPI value for each feature, i.e. how much did the loss function change when the feature was replaced with its knockoff version, with corresponding standard errors, test statistics, p-values and confidence interval. Note that if the CPI value is exactly 0 (e.g. if a feature was never selected for splitting in a tree), the statistical test returns `NA`. 
...
Here we see more `NA` values because the classification error is less sensitive to small changes and hence results in lower power. 
